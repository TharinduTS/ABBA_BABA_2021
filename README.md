...
run it the same way
```bash
for i in all l_only s_only; do cd ${i};  perl ../scripts/jackknife_for_fdm.pl autosomes_BW_Laigns_DCGV_KD.csv > jack_fdm_BW_Laigns_DCGV_KD.tsv;  perl ../scripts/jackknife_for_fdm.pl autosomes_KD_Threesis_BW_DCGV.csv > jack_fdm__KD_Threesis_BW_DCGV.tsv;  cd .. ; done
```
And collect them
```bash
mkdir ../jack_fdm_to_download
find . -name 'jack_fdm*.tsv' -exec cp --parents \{\} ../jack_fdm_to_download/ \;
cd ..
```
